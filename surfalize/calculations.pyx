...
def _surface_area_gwyddion_cy(double[:,:] p, double dx, double dy):
    """
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
def _surface_area_iso_cy(double[:,:] p, double dx, double dy):
    """
    Calculates the surface area of a 3d pointcloud with regular grid spacing in x and y. In each set of four
...
def surface_area(array, dx, dy, method='iso'):
    ...
    """
    if method == 'iso':
        return _surface_area_iso_cy(array, dx, dy)
    if method == 'gwyddion':
        extended_array = _extend_array(array)
        return _surface_area_gwyddion_cy(extended_array, dx, dy)
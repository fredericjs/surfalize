...
@functools.cache
def get_possible_mu_decodings():
    """
    Generate all possible incorrect decodings of the 'µ' character across a set of encodings.

    This function attempts to encode and decode the 'µ' (micro) symbol using every pair
    of encodings from the global `ENCODINGS` list. If decoding does not raise an exception,
    the result is added to a set of possible "incorrect" decodings.

    Returns
    -------
    set of str
        A set of strings representing potentially misdecoded versions of the 'µ' character.
    """
    decodings = set()
    for ec1 in ENCODINGS:
        for ec2 in ENCODINGS:
            try:
                decodings.add('µ'.encode(ec1).decode(ec2))
            except Exception:
                pass
    return decodings

def fix_decoding_issues(unit):
    """
    Fix units that may have incorrect encodings of the 'µ' (micro) symbol.

    If a string ends with 'm' and the preceding characters match any of the known
    incorrect decodings of the 'µ' symbol, it replaces the prefix with 'u' to form 'um'.

    Parameters
    ----------
    unit : str
        The input unit string potentially containing a misdecoded micro symbol.

    Returns
    -------
    str
        The corrected unit string, with known misdecodings replaced by 'u'.
    """
    if unit.endswith('m'):
        symbol = unit[:-1]
        if symbol in get_possible_mu_decodings():
            unit = 'um'
    return unit
...
def get_unit_conversion(from_unit, to_unit):
    ...
    if from_unit not in UNIT_EXPONENT:
        from_unit = fix_decoding_issues(from_unit)
    if to_unit not in UNIT_EXPONENT:
        to_unit = fix_decoding_issues(to_unit)
    if from_unit not in UNIT_EXPONENT:
        raise ValueError(f'Unit {from_unit} does not exist.')
    if to_unit not in UNIT_EXPONENT:
        raise ValueError(f'Unit {to_unit} does not exist.')
